/**
 * Memory Node — Store/retrieve workflow data
 */
...
const statusColors: Record<string, string> = {
  idle: '#a855f7', running: '#f59e0b', success: '#10b981',
  error: '#ef4444', warning: '#f97316', cancelled: '#64748b',
}

export const MemoryNode: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontWeight: 600, color: '#f1f5f9', flex: 1 }}>{data.label || 'Memory'}</span>
      ...
      <div style={{ padding: '10px 12px' }}>
        <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
          <span style={badge('#a855f7')}>{data.memoryType || 'short-term'}</span>
          <span style={badge('#64748b')}>{data.storage || 'local'}</span>
        </div>
        {data.key && (
          <div style={{ fontSize: 10, color: '#94a3b8' }}>
            Key: <span style={{ color: '#c4b5fd', fontFamily: 'monospace' }}>{data.key}</span>
          </div>
        )}
      </div>
